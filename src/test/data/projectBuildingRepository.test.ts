...
const details = {
  ...
  planned_shops: 3, planned_offices: 0, planned_parking_spaces: 10,
  planned_houses: null,
...
describe("project building details", () => {
  ...
  it("saves planned counts and areas for one project", async () => {
    ...
    vi.spyOn(client, "query").mockResolvedValue([{
      id: "building-1", ...details, planned_flats: 5, has_masjid: 1,
    ...
    expect(execute).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([projectId, "mixed-use", 2, 5, 3, 10, "marla"]),
    );
  });
});
